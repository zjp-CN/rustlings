...
// type. To get around the issue, we can use a `Box` - a smart pointer used to store data on
// the heap, which also allows us to wrap a recursive type.
//
...
mod tests {
    ...
    #[test]
    fn test_create_empty_list() { assert_eq!(List::Nil, create_empty_list()) }
